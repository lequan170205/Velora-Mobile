...
```tsx
import { useLLM, LLAMA3_2_1B, MessageCountContextStrategy } from 'react-native-executorch'
...
llm.configure({
  chatConfig: {
    ...
    initialMessageHistory: [{ role: 'user', content: 'What is the current time and date?' }],
    contextStrategy: new MessageCountContextStrategy(6),
  },
})
```
...
```tsx
<Button title="Stop" onPress={() => llm.interrupt()} disabled={!llm.isGenerating} />
```
