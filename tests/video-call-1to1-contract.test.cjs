...
test('conversation video entry point remains direct-chat only', () => {
  ...
  assert.match(screen, /const \{ startVideoCall, startVoiceCall, joinGroupCall \} = useCall\(\)/)
...
test('minimized calls use native resume surfaces and a draggable global return control', () => {
  ...
  assert.match(floatingCallButton, /width - insets\.right - EDGE_INSET - buttonWidth/)
  ...
  assert.match(floatingCallButton, /className="h-12 flex-row/)
  assert.match(floatingCallButton, /const buttonWidth = isGroupCall \? 96 : BUTTON_SIZE/)
  ...
  assert.match(screen, /callPhase === 'idle' &&\s*!serverCall &&\s*\(currentConversation\?\.isGroup === true/)
