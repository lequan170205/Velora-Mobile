...
on_error() {
  ...
  echo "[Velora CI] FAILED stage=$CI_STAGE line=$line exit=$status command=$command" >&2
  exit "$status"
}
trap on_error ERR
...
if [[ ! -s "$FIREBASE_IOS_PLIST" ]]; then
  echo "[Velora CI] Firebase plist disappeared before archive: $FIREBASE_IOS_PLIST" >&2
  exit 1
fi
