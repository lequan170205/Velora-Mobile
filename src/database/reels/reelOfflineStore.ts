...
const applyCachedReelInput = (record: CachedReelModel, input: CachedReelInput) => {
  ...
  record.mediaProgress = input.mediaProgress
  record.indexStage = input.indexStage
  record.indexProgress = input.indexProgress
  record.sourceDurationMs = input.sourceDurationMs
  record.sourceOrientation = input.sourceOrientation
  record.sourceLengthClass = input.sourceLengthClass
  record.sourceAspectRatio = input.sourceAspectRatio
  record.sourceEffectiveWidth = input.sourceEffectiveWidth
  record.sourceEffectiveHeight = input.sourceEffectiveHeight
  record.hlsMasterUrl = input.hlsMasterUrl
  record.captionVttUrl = input.captionVttUrl
